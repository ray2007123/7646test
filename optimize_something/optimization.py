...
def get_portfolio(prices, alloc, initial_amount):
    ...
    portfolio_values = (prices_normalized * alloc * initial_amount).sum(axis=1)
    return portfolio_values

def cal_daily_returns(portfolio_values):
    daily_returns = portfolio_values.pct_change()
    daily_returns = daily_returns.dropna()  # Drop NaN value introduced by pct_change
    return daily_returns

def cal_avg_daily_returns(daily_returns):
    dr = daily_returns.mean()
    return dr
...
def sum_cummulative_returns(portfolio_values):
    pv1 = portfolio_values[-1]
    pv2 = portfolio_values[0]
    cr = (pv1 / pv2) - 1
    return cr

def opt_sharpe_ratio(daily_returns):
    adr = cal_avg_daily_returns(daily_returns)
    sddr = cal_std_daily_returns(daily_returns)
